...
public class LockInternals {
    ...
    private final Watcher watcher = new Watcher() {
        ...
        public void process(WatchedEvent event) {
            System.out.println(LocalDateTime.now() + " 收到 event: " + JSON.toJSONString(event));
    ...
    String attemptLock(long time, TimeUnit unit, byte[] lockNodeBytes) throws Exception {
        ...
        while (!isDone) {
            ...
            try {
                ...
                System.out.println("创建临时有序节点: " + ourPath);
                //上锁
                //1.如果自己是第一个节点  则获取锁成功
                //2.如果不是   则监听自己前面的节点
                ...
                if (predicateResults.getsTheLock()) {
                    ...
                    synchronized (this) {
                        try {
                            ...
                            System.out.println("监听: " + previousSequencePath);
                            ...
                            if (millisToWait != null) {
                                ...
                                //lock(时间)  指定时间内wait  再获取一次 获取不成功则失败
                                wait(millisToWait);
                            } else {
                                //lock() 一直等待
                                wait();
